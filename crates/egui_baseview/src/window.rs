...
impl<State, U> EguiWindow<State, U>
    where State: 'static + Send, U: FnMut(&egui::Context, &mut Queue, &mut State), U: 'static + Send
{
    fn new<B>(
        ...
        -> EguiWindow<State, U>
        where B: FnMut(&egui::Context, &mut Queue, &mut State), B: 'static + Send
    {
        ...
        let egui_input = egui::RawInput {
            screen_rect: Some(
                egui::Rect::from_points(
                    &[
                        Default::default(),
                        [(physical_width as f32) / scale, (physical_height as f32) / scale].into(),
                    ]
                )
            ),
            viewports: [
...
impl<State, U> WindowHandler
    for EguiWindow<State, U>
    where State: 'static + Send, U: FnMut(&egui::Context, &mut Queue, &mut State), U: 'static + Send
{
    fn on_frame(&mut self, window: &mut Window) {
        if let Some(state) = &mut self.user_state {
            ...
            let do_repaint_now = if let Some(t) = self.repaint_after {
                now >= t || self.repaint_after.is_none()
            } else {
        ...
        match &event {
            baseview::Event::Mouse(event) =>
                match event {
                    ...
                    baseview::MouseEvent::WheelScrolled { delta: scroll_delta, modifiers } => {
                        self.update_modifiers(modifiers);
                        ...
                        let mut delta = match scroll_delta {
                            ...
                            baseview::ScrollDelta::Pixels { x, y } => {
                                if
                                    let Some(pixels_per_point) = self.egui_input.viewports
                                        .get(&ViewportId::ROOT)
                                        .map(|v| v.native_pixels_per_point)
                                {
                                    egui::vec2(*x, *y) / pixels_per_point.unwrap()
                                } else {
                                    egui::vec2(*x, *y)
                                }
                            }
                        ...
                        if self.egui_input.modifiers.ctrl || self.egui_input.modifiers.command {
                            ...
                            self.egui_input.events.push(
                                egui::Event::Scroll(egui::vec2(delta.x + delta.y, 0.0))
                            );
                        } else {
            ...
            baseview::Event::Keyboard(event) => {
                ...
                if pressed {
                    ...
                    if is_cut_command(self.egui_input.modifiers, event.code) {
                        ...
                        if !self.egui_input.modifiers.ctrl && !self.egui_input.modifiers.command {
                            self.egui_input.events.push(egui::Event::Text(written.clone()));
                        }
                    }
                }
            }
            baseview::Event::Window(event) =>
                match event {
                    baseview::WindowEvent::Resized(window_info) => {
                        ...
                        if
                            let Some(viewport) = self.egui_input.viewports.get_mut(
                                &ViewportId::ROOT
                            )
                        {
                            viewport.native_pixels_per_point = Some(self.scale_factor);
                        }

                        self.egui_input.screen_rect = Some(
                            Rect::from_min_size(
                                Pos2::new(0f32, 0f32),
...
fn is_cut_command(modifiers: egui::Modifiers, keycode: keyboard_types::Code) -> bool {
    (modifiers.command && keycode == keyboard_types::Code::KeyX) ||
        (cfg!(target_os = "windows") && modifiers.shift && keycode == keyboard_types::Code::Delete)
}
...
fn is_copy_command(modifiers: egui::Modifiers, keycode: keyboard_types::Code) -> bool {
    (modifiers.command && keycode == keyboard_types::Code::KeyC) ||
        (cfg!(target_os = "windows") && modifiers.ctrl && keycode == keyboard_types::Code::Insert)
}
...
fn is_paste_command(modifiers: egui::Modifiers, keycode: keyboard_types::Code) -> bool {
    (modifiers.command && keycode == keyboard_types::Code::KeyV) ||
        (cfg!(target_os = "windows") && modifiers.shift && keycode == keyboard_types::Code::Insert)
}
